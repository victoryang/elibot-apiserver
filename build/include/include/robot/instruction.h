...
typedef struct {
	STRU_CONST_VAR dist; 		// 距离
	STRU_CONST_VAR r; 			// 半径
	STRU_CONST_VAR  direction;	// 方向
    unsigned char point_num;    // 点号
    STRU_MOV common;
}STRU_MOVCA;
...
int get_iobus_io_var_internal(char *name);
int set_iobus_io_var_internal(char *name, int value);
int get_iobus_io_type_internal(char *name);
