...
#include <robot/track.h>
#include <robot/visiontrack.h>
#include <sys/mman.h>
...
#define setModualFunc(v)	do{NVRAM_DATA(sys_technics)=v;msync_NVRAM();}while(0)
...
typedef enum {
	...
	st_save_arcwelddata_success = 14,
}SYSCOMM_STATE;

typedef struct
{
	//unsigned char extAxisEnable;//extern axis disable/enable(Disable = 0 ; Enable = 1)
	unsigned char coordAxis;//(手动时,1:协同一轴 2:协同二轴)/(自动时, 0:没有 1:协同一轴 2:协同二轴 3:全部协同)
	unsigned char if_teached[EXTAXIS_COUNT];
	unsigned char dirct[EXTAXIS_COUNT];
	unsigned char coordEnable[EXTAXIS_COUNT];//coordinate switch disable/enable(Disable = 0 ; Enable = 1)
	unsigned char caliSuccess[EXTAXIS_COUNT];//Has it been calibrated successfully (fail = 0 ; success = 1)
...
typedef struct{
	...
	Track_parameter trackpara;
	VisionTrackData vitrackpara;
...
#define SHARE_RES(x)	share_resource->x

int resource_create(const char *key_file);
int resource_init(const char *key_file);
...
extern struct para_info sys_parameters;
...
#define NVRAM_DATA(x)	nvram_data->data.x
//#define NVRAM_PARA(x)nvram_data->para.data.x
#define NVRAM_PARA(x)	sys_parameters.data.x

#define NVRAM_PARA_DATA_POINTER() &(sys_parameters.data)
#define NVRAM_PARA_POINTER() &(sys_parameters)
//#define msync_NVRAM()	do{msync(nvram_data, NVRAM_SIZE, MS_SYNC);printf("set data msync:%d %s\n",__LINE__,__FUNCTION__);}while(0)
...
#define SETMOTOR_RUN_STATE(v)	do{if(SHARE_RES(can_motor_run)!=v) SHARE_RES(can_motor_run)=v;}while(0)
...
#define SetMainfile(fname)		do{sprintf( NVRAM_DATA(projectName),"%s",fname);printf("ser= %s\n",GetMainfile());}while(0)
...
#define SetManualSpeedRate(v)	do{NVRAM_DATA(manualSpeedRate)=(v>2000?2000:v);msync_NVRAM();}while(0)
...
#define STRU_ARCWELD_POINT()	(&share_resource->stru_arcWeld)
#define STRU_ARCWELD_DATA(d)	(share_resource->stru_arcWeld.d)
...
#define GetDisplaySerialNum()	SHARE_RES(dspInfor).serial_num
#define SetDisplaySerialNum(msg)	memcpy(SHARE_RES(dspInfor).serial_num, msg, 33)

#define DisplayTotalTime()	SHARE_RES(dspInfor).total_time
#define DisplayRunTime()	SHARE_RES(dspInfor).run_time
