...
cJSON* convertToIntArray(uint8_t* base, int array_size) {
	...
	for (;i<array_size;i++) {
		cJSON_AddItemToArray(array, cJSON_CreateNumber(base[i]));
	}
	return array;
}
cJSON* getRobPLC() {
...
cJSON* getSysvar() {
	...
	cJSON_AddItemToObject(item, "cRobB", cJSON_CreateIntArray(SHARE_RES(sysvar).cRobB, B_COUNT));
...
cJSON* getLocvar() {
	...
	cJSON_AddItemToObject(item, "cRobLB", cJSON_CreateIntArray(SHARE_RES(sysvar).cRobLB, LB_COUNT));

	cJSON_AddItemToObject(item, "iRobLI", cJSON_CreateIntArray(SHARE_RES(sysvar).iRobLI, LI_COUNT));

	cJSON_AddItemToObject(item, "dRobLD", cJSON_CreateDoubleArray(SHARE_RES(sysvar).dRobLD, LD_COUNT));

	array = cJSON_CreateArray();
	for (i=0; i<LP_COUNT; i++) {
		cJSON_AddItemToArray(array, cJSON_CreateDoubleArray(SHARE_RES(sysvar).dRobLP[i], AXIS_COUNT))
	}
	cJSON_AddItemToObject(item, "dRobLP", array);

	array = cJSON_CreateArray();
	for (i=0; i<LV_COUNT; i++) {
		cJSON_AddItemToArray(array, cJSON_CreateDoubleArray(SHARE_RES(sysvar).dRobLV[i], VSub_COUNT))
	}
